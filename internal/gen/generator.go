...
type BulkFunc struct {
	...
	ParamsType string
	// InsertPrefix is the raw SQL text up to and including "VALUES ", e.g.:
	//   "INSERT INTO users (name, email) VALUES "
	//   "INSERT INTO `users` (`name`, `email`) VALUES "
	// The template quotes this via the "quote" FuncMap entry (strconv.Quote).
	InsertPrefix string
...
// The "quote" template function (strconv.Quote) is registered via FuncMap so
// that InsertPrefix — which may contain backtick-quoted MySQL identifiers — is
// safely emitted as a double-quoted Go string constant.
...
{{range .Funcs}}
const {{.ConstName}} = {{.InsertPrefix | quote}}
...
// bulkInsertTemplate is the parsed, ready-to-execute form of bulkInsertTmpl.
// template.Must panics on a syntax error, surfacing bugs in the constant
// template string immediately at init rather than silently at generation time.
var bulkInsertTemplate = template.Must(
	template.New("bulk").
		Funcs(template.FuncMap{"quote": strconv.Quote}).
		Parse(bulkInsertTmpl),
)
...
func renderFile(pkg string, funcs []BulkFunc) ([]byte, error) {
	...
	// Pre-grow to avoid repeated doubling for typical function sizes (~512 B each).
	var buf bytes.Buffer
	buf.Grow(512 * len(funcs))
...
func buildBulkFunc(q *plugin.Query) (BulkFunc, error) {
	...
	if n == 1 {
		...
		for i, p := range q.Params {
			if p == nil || p.Column == nil {
				return BulkFunc{}, fmt.Errorf("param %d has nil column", i+1)
			}
